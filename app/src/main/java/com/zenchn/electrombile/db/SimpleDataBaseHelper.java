package com.zenchn.electrombile.db;
...
/**
 * Created by wangr on 2016/11/16 0016.
 */
public class SimpleDataBaseHelper extends DataBaseHelper {

    private static SimpleDataBaseHelper mSimpleDataBaseHelper;

    private SimpleDataBaseHelper(Context context) {
        super(context);
    }

    public static SimpleDataBaseHelper getInstance(Context context) {
        if (mSimpleDataBaseHelper == null) {
            synchronized (DataBaseHelper.class) {
                if (mSimpleDataBaseHelper == null) {
                    mSimpleDataBaseHelper = new SimpleDataBaseHelper(context);
                    if (mSimpleDataBaseHelper.getDB() == null || !mSimpleDataBaseHelper.getDB().isOpen()) {
                        mSimpleDataBaseHelper.open();
                    }
                }
            }
        }
        return mSimpleDataBaseHelper;
    }
    ...
    protected String[] getDbCreateSql(Context context) {
        ...
        a[0] = "CREATE TABLE location (id INTEGER PRIMARY KEY AUTOINCREMENT,utcTime bigint,lon varchar,lat varchar,speed REAL,acc INTEGER)";
        a[1] = "CREATE TABLE location_contrail_current (id INTEGER PRIMARY KEY AUTOINCREMENT,utcTime bigint,lon varchar,lat varchar,speed REAL,acc INTEGER)";
        a[2] = "CREATE TABLE location_contrail (id INTEGER PRIMARY KEY AUTOINCREMENT,utcTime bigint,lon REAL,lat varchar,speed varchar,acc INTEGER)";
        return a;
    }
