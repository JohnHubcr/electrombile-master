...
public class VehicleInfoApi extends BaseApi {
     ...
     * @param equModel
     */
    public void getVehicleStatus(String accessToken, String serialNumber, final int equModel, final VehicleCheckCallback callback) {

        mApiStore
                .getVehicleStatus(accessToken, serialNumber, String.valueOf(equModel))
                .enqueue(new Callback<String>() {
                             @Override
                             public void onResponse(Call<String> call, Response<String> response) {
                                 try {
                                     ...
                                     if (response.code() == 200) {
                                         ...
                                         if (EquModelUtils.EQU_MODEL_ST == equModel) {

                                             VehicleCheckRawForSTInfo vehicleCheckRawForSTInfo = new VehicleCheckRawForSTInfo();
                                             vehicleCheckRawForSTInfo.setBmsTrouble(data.getString("bmsStatus"));
                                             vehicleCheckRawForSTInfo.setVehicleConditionStatus(data.getString("vehicleInfo"));


                                             LogKit.success("车辆自检信息(神腾)获取成功", vehicleCheckRawForSTInfo.toString());
                                             callback.onGetVehicleCheckSuccess(vehicleCheckRawForSTInfo);

                                         } else {
                                             VehicleCheckRawForJKInfo vehicleCheckRawForJKInfo = new VehicleCheckRawForJKInfo();

                                             vehicleCheckRawForJKInfo.setBmsTrouble(data.getString("bmsTrouble"));
                                             vehicleCheckRawForJKInfo.setChargerTrouble(data.getString("chargerTrouble"));
                                             vehicleCheckRawForJKInfo.setControlTroubleStatus1(data.getString("controlTroubleStatus1"));
                                             vehicleCheckRawForJKInfo.setControlTroubleStatus2(data.getString("controlTroubleStatus2"));
                                             vehicleCheckRawForJKInfo.setEcuOnlineCheck(data.getString("ecuOnlineCheck"));
                                             vehicleCheckRawForJKInfo.setEcuTrouble(data.getString("ecuTrouble"));
                                             vehicleCheckRawForJKInfo.setGyroscopeStatus(data.getString("gyroscopeStatus"));
                                             vehicleCheckRawForJKInfo.setMeterTroubleStatus(data.getString("meterTroubleStatus"));

                                             LogKit.success("车辆自检信息(金开)获取成功", "定位信息：\n" + vehicleCheckRawForJKInfo.toString());
                                             callback.onGetVehicleCheckSuccess(vehicleCheckRawForJKInfo);
                                         }

                                     } else if (response.code() == 401) {

                                         callback.onGrantRefuse();
                                         LogKit.success("车辆自检信息获取失败", "失败原因：\n" + JSON.parseObject(response.errorBody().string()).getString("error"));

                                     } else {
                                         JSONObject errorResult = JSON.parseObject(response.errorBody().string());
                                         LogKit.success("车辆自检信息获取失败", "失败原因：\n" + errorResult.getString("error"));
                                         callback.onGetVehicleCheckResponseError(errorResult.getString("error"));
                                     }

                                 } catch (Exception e) {
                                     callback.onGetVehicleCheckFailure();
                                     LogKit.exception("车辆自检信息获取失败", "解析错误，异常信息：\n" + e.toString());
                                 }
                             }
                             ...
                             public void onFailure(Call<String> call, Throwable t) {
                                 callback.onGetVehicleCheckFailure();
                                 LogKit.exception("车辆自检信息获取失败", "访问失败, 异常信息：\n" + t.toString());
                             }
                         }
