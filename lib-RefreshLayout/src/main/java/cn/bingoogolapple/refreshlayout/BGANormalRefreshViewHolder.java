...
public class BGANormalRefreshViewHolder extends BGARefreshViewHolder {
    ...
    private String mPullDownRefreshFinishText = "加载完成";

    private String mPullDownRefreshFinishTimeFormat = "yyyy-MM-dd HH:mm:ss";
    ...
    public void setmPullDownRefreshFinishTimeFormat(String mPullDownRefreshFinishTimeFormat) {
        this.mPullDownRefreshFinishTimeFormat = mPullDownRefreshFinishTimeFormat;
    }

    /**
     * 获取当前时间(可自定义：默认为yyyy-MM-dd HH:mm:ss)
     *
     * @return
     */
    private String getStandardCurrentTime() {
        String currentTime = null;
        try {
            currentTime = new SimpleDateFormat(mPullDownRefreshFinishTimeFormat, Locale.CHINA).format(new Date());
        } catch (Exception e) {//防止使用者传入错误的日期格式
            this.mPullDownRefreshFinishTimeFormat = "yyyy-MM-dd HH:mm:ss";
            currentTime = new SimpleDateFormat(mPullDownRefreshFinishTimeFormat, Locale.CHINA).format(new Date());
        } finally {
            return currentTime;
        }
    ...
    @Override
    public void changeToRefreshFinish() {
        mHeaderStatusTv.setText(getStandardCurrentTime() + " " + mPullDownRefreshFinishText);
        mHeaderChrysanthemumIv.setVisibility(View.INVISIBLE);
